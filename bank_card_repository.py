...
class BankCardRepository:
    ...
    def correct_date(self, date: str) -> str:
        ...
        return date[-4:] + '-' + date[:2] + first_day_of_month  # Перетворення формату дати
    ...
    def __init__(self):
        try:
            ...
            print(f"[Error] Помилка при роботі з PostgreSQL \n {_ex}")  # Обробка помилки підключення
    ...
    def save(self, Card):
        self.cursor.execute('INSERT INTO cards VALUES(%s, %s, %s, %s, %s, %s)',
                            Card.attributes())  # Додавання даних до таблиці

    def update_status(self, Card):
        self.cursor.execute('UPDATE cards SET card_status = %s WHERE card_number = %s',
                            (Card.card_status, Card.card_number))  # Оновлення статусу картки

    def find_by_exp_date(self, date: str) -> tuple:
        formatted_date = self.correct_date(date)
        self.cursor.execute('SELECT * FROM cards WHERE card_exp_date = %s', (formatted_date,))
        return self.cursor.fetchall()  # Отримання та повернення всіх рядків, що відповідають вказаній даті закінчення строку дії

    def find_by_issue_date(self, issue_date: str) -> tuple:
        self.cursor.execute('SELECT * FROM cards WHERE card_issue_date = %s', (issue_date,))
        return self.cursor.fetchall()  # Отримання та повернення всіх рядків, що відповідають вказаній даті видачі
...
if __name__ == '__main__':
    ...
    card1 = Card(1234_5678_9012_3456, '02/2022', 123, '2028-02-01', str(uuid4()), 'active')

    # Збереження картки в базі даних
    #bank.save(card1)

    # Отримання картки за номером
    print(bank.get(1234_5678_9012_3456))
