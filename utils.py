...
#------------------------------------Домашнее задание---------------------------------------
def get_delimiter(date: str) -> str:
    # Функция определяет разделитель даты
    if date[2] == date[-5]:
        return date[2]
    else:
        return date[4]
...
def check_bank(bank: str) -> str:
    # Функция проверяет формат названия банка и возвращает правильный вариант, либо ошибку, если такого банка не существует
    if bank.lower() in ('nbu', 'nationalbank', 'nb'):
        return 'NBU'
    elif bank.lower() in ('pb', 'privatbank'):
        return 'PB'
    else:
