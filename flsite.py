...
def check_issue_date(date) -> bool:
    date = date.replace('-', '')
    if len(date) == 8 and date.isdigit() and 1 <= int(date[4:6]) <= 12 and 1 <= int(date[6:]) <= 31:
        return False
    else:
        return True
...
def create():
    ...
    if request.method == 'POST':
        if check_number(number := request.form['number']):
            ...
            number = number.replace('-','')
            card1 = Card(int(number), exp_date, int(cvv), issue_date, str(uuid4()), 'new')
            try:
                rep = BankCardRepository(host, user, password, db_name)
                rep.save(card1)
    ...
    return render_template('сreate.html', title='Створення картки', error=error, success=success)
